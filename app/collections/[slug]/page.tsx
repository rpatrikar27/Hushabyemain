...
export default function CollectionPage() {
  ...
  const products = slug === 'all' 
    ...
    : seedData.products.filter(p => {
        ...
        return cat?.slug === slug || p.tags?.includes(slug as string);
      });

  return (
    <div className="flex min-h-screen flex-col">
      <SEOHead 
        title={category?.name || (slug === 'all' ? 'All Products' : slug?.toString())} 
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
        <nav className="flex items-center gap-2 text-sm text-slate-500 mb-8">
          ...
          <span className="text-slate-900 font-medium capitalize">{slug?.toString().replace('-', ' ')}</span>
        </nav>
